...
<?php 
	...
	$parent      = (isset($this->arrParam['form']['parent'])) ? $this->arrParam['form']['parent'] : null;
	//Input
	...
	$selectParent 	= Helper::cmsSelectbox('form[parent]', 'form-control', $this->slbCategory, $parent, null, 'theSelect');
	...
	$rowParent	= Helper::cmsRowForm('Parent', $selectParent, 'col-sm-3', 'col-sm-9');
 ?>
  ...
  <div class="box-body">
   <div class="row">
      ...
      <form class="form-horizontal" id="myForm" action="" method="POST" enctype="multipart/form-data">
        <?php echo $rowParent.$rowName.$rowCatecode.$rowStatus.$rowShowOnFE.$rowShowOnFooter.$rowPosition. $rowPositionFooter. $rowTitle. $rowDes.$inputIDHidden.$inputHidden ?>
      </form> 
    </div>
   </div>
  </div>
...
<script type="text/javascript">
    $(function() {
        ...
        var patt1 = /(\d+)/;
        var result = location.match(patt1);
        var id       = result[1];
        $("#theSelect option[value=" + id + "]").hide();
    });
</script>