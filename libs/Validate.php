...
class Validate{
	...
	public function run(){
		foreach($this->rules as $element => $value){
			if($value['required'] == true && trim(@$this->source[$element])==null){
				$this->setError($element, 'value is not empty!!');
			}else{
				switch ($value['type']) {
					case 'int':
	...
	public function showErrors(){
		...
		if(!empty($this->errors)){
			$xhtml .= '<div class="row"><div class="col-md-6 col-md-offset-3"><div class="alert alert-danger alert-dismissable text-justify"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>';
	...
	public function showErrorsLogin(){
		$xhtml = '';
		if(!empty($this->errors)){
			$xhtml .= '<div class="alert alert-danger alert-dismissable text-justify"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>';
			foreach($this->errors as $key => $value){
				$xhtml .= $value.'<br/>';
			}
			$xhtml .=  '</div>';
	...
	private function validateStatus($element, $options){
		if(in_array($this->source[$element], $options['deny']) == true){
			$this->setError($element, 'Please select a value other than the default value!');
		}
	}
	...
	private function validateExistRecord($element, $options){
		...
		if($database->isExist($query)==false){
			$this->setError($element, 'value does not exist!');
		}
	}
