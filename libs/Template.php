...
class Template{
	...
	public function load(){
		...
		if(file_exists($pathFileConfig)){
			...
			$view->_title 		= $view->createTitle((!empty($arrCongif['title']) ? $arrCongif['title'] : ''));
			$view->_metaHTTP 	= $view->createMeta(!empty($arrCongif['metaHTTP']) ? $arrCongif['metaHTTP'] : '', 'http-equiv');
			$view->_metaName 	= $view->createMeta(!empty($arrCongif['metaName']) ? $arrCongif['metaName'] : '', 'name');
			$view->_cssFiles 	= $view->createLink($this->_folderTemplate . (!empty($arrCongif['dirCss']) ? $arrCongif['dirCss'] : ''), (!empty($arrCongif['fileCss']) ? $arrCongif['fileCss'] : ''), 'css');
			$view->_jsFiles 	= $view->createLink($this->_folderTemplate . (!empty($arrCongif['dirJs']) ? $arrCongif['dirJs'] : ''), (!empty($arrCongif['fileJs']) ? $arrCongif['fileJs'] : ''), 'js');
			$view->_dirImg 		= TEMPLATE_URL . $this->_folderTemplate . (!empty($arrCongif['dirImg']) ? $arrCongif['dirImg'] : '');
